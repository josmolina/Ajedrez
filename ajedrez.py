...
import random
import time
from Hash import HashZobrist


profundidad = 5
zobrist = HashZobrist()
tablero = chess.Board()
vN = {0: zobrist.tableroHash(tablero)}
TTHash = {zobrist.tableroHash(tablero): [0, profundidad, True]}
contador = 1
...
class Juego(object):
    ...
    def seleccionaMovimiento(self, nivel):
        if nivel == 0:
            ...
            return self.mejorMovimiento3(vN, TTHash, contador, 5)
    ...
    def negamax3(self, alfa, beta, profundidad, zobrist, vN, TTHash, contador, MaxMin):
        maxEval = -999999
        if profundidad == 0:
            return self.quiesce(alfa, beta)
        if MaxMin:
            maxEval = -999999
            for movimiento in self.tablero.legal_moves:
                hash = Hash.reiniciarHash(tablero, movimiento)
                if hash in TTHash and TTHash[has][1] >= profundidad and TTHash[hash][2] == False:
                    Hash.reiniciarHash(tablero, movimiento)
                    maxEval = max(maxEval, TTHash[hash][0])
                else:
                    self.tablero.push(movimiento)
                    valorEval = -(self.negamax3(-beta, -alfa, profundidad-1, zobrist, vN, TTHash, contadorm ,MaxMin))
                    TTHash[hash] = [valorEval, profundidad -1, -MaxMin]
                    vN[contador] = hash
                    contador += 1
                    self.tablero.pop()
                    maxEval = max(maxEval, valorEval)
    ...
    def mejorMovimiento3(self, vN, TTHash, contador, profundidad):
        try:
            movimiento = chess.polyglot.MemoryMappedReader(
